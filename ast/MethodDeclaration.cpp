...
MethodDeclaration::setParameters(std::vector<Declaration *> *ps) {
    ...
    for (int i = 0; i < ps->size(); i++)
        if (ps->at(i) == NULL)
            ...
            throw AstException(
                    "MethodDeclaration::setParameters: one of the Declaration pointers is not a ParameterDeclaration\n");
...
MethodDeclaration::setVariables(std::vector<Declaration *> *vs) {
    ...
    for (int i = 0; i < vs->size(); i++)
        if (vs->at(i) == NULL)
            ...
            throw AstException(
                    "MethodDeclaration::setVariables: one of the Declaration pointers is not a VariableDeclaration\n");
