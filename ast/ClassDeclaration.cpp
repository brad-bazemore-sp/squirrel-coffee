...
ClassDeclaration::ClassDeclaration(int lno, const char *n) {
    ...
    if (members == NULL)
        throw AstException(
                "Internal error: ClassDeclaration::ClassDeclaration: failed to create members std::vector\n");
}
...
ClassDeclaration::addMember(Declaration *member) {
    ...
    if (members == NULL) {
        ...
        if (member->getKind() != AstNode::DFIELD &&
            ...
            throw AstException(
                    "ClassDeclaration::addMember: illegal argument type; must be a pointer to FieldDeclaration or MethodDeclaration");
